...
% 
% io_loadspec_niimrs outputs the data in structure format, with fields corresponding to time
...
nii_data = nii_tool('load', filename);
...
f=[(-sw/2)+(sw/(2*sz(1))):sw/(sz(1)):(sw/2)-(sw/(2*sz(1)))];
ppm=-f/(Bo*42.577);
ppm=ppm+4.65;
...
for rr = 1:length(out.nucleus)
    ...
    out.Bo(rr) = f0{rr} ./ gamma;
end

% Echo/repetition/inversion/mixing time
out.te = hdr_ext.EchoTime;
out.tr = hdr_ext.RepetitionTime;
out.ti = hdr_ext.InversionTime;
out.tm = hdr_ext.MixingTime;
